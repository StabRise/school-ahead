...
def _assignment_out(assignment: TutorSubjectAssignment, request: HttpRequest) -> AssignmentOut:
    return AssignmentOut(
        ...
        subject_icon=icon_url(assignment.subject, request),
...
def upload_tutor_subject_icon(request: HttpRequest, subject_id: int, file: UploadedFile = File(...)):
    ...
    # Named after the icon it belongs to and made before that changes — nothing
    # is generated by asking for it.
    previous_thumbnail = subject.icon_thumbnail if previous_icon else None
    subject.icon.save(file.name, file, save=True)
    if previous_icon:
        subject.icon.storage.delete(previous_icon)
        previous_thumbnail.storage.delete(previous_thumbnail.name)
    return subject
...
def list_lessons_needing_review(request: HttpRequest, subject: int | None = None, class_id: int | None = None):
    ...
    return [
        NeedReviewLessonOut(
            ...
            subject_icon=icon_url(lesson.topic.subject, request),
