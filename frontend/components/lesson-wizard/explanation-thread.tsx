...
export function ExplanationThread({ comments }: { comments: LessonCommentOut[] }) {
          ...
          return (
            <div key={question.id} className="flex flex-col gap-2 rounded-md border border-gray-200 p-3">
              ...
              {replies.length > 0 ? (
                ...
                <p className="rounded-md border border-dashed border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-400">
                  {t("pendingReplyPlaceholder")}
                </p>
              )}
            </div>
          );
