...
// real table: icon / subject+block (flexible) / progress bar / percent.
...
function SimpleSubjectRow({ subject }: { subject: SubjectOut }) {
    ...
    <li>
      <Link href={`/subjects/${subject.id}`} className={`${ROW_GRID} px-2 py-2 hover:bg-gray-50`}>
        ...
        <span className="min-w-0 truncate">
          ...
          {activeBlock && <span className="text-gray-400"> · </span>}
          {activeBlock && <span className="text-xs text-gray-500">{activeBlock.label}</span>}
        </span>
...
// grid — a dense, borderless list of subject rows instead of shadowed
// cards, matching the Simple dashboard/calendar's visual language. See the
      ...
      {subjects.length > 0 && (
        <div className="overflow-x-auto">
          <div className={`${ROW_GRID} min-w-[28rem] px-2 pb-2`}>
            ...
            <span className="text-xs font-medium text-gray-500">{t("columnSubject")}</span>
            <span className="text-xs font-medium text-gray-500">{t("progressLabel")}</span>
            <span aria-hidden="true" />
          </div>
          <ul className="min-w-[28rem] divide-y divide-gray-100">
            {subjects.map((subject) => (
              <SimpleSubjectRow key={subject.id} subject={subject} />
            ))}
          </ul>
        </div>
      )}
