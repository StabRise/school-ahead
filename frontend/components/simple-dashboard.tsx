...
// A today lesson or a backlog ("tail") lesson merged into one row — every
// CalendarItemOut field plus the backlog-only origin_label, so both
// BacklogItemOut and CalendarItemOut values fit this shape with no
// per-item mapping.
type SimpleRow = CalendarItemOut & { origin_label?: string };
...
// same field student-dashboard.tsx's Standard view already formats as its
...
// Notion-style, monochrome (no colored badges, no card borders) alternative
// to the Standard dashboard's card list — see the Settings page's "Вигляд"
// section (components/settings/view-settings.tsx). Merges today's lessons
// and the backlog into one flat, sortable table rather than the Standard
// view's two separate sections — a flat sortable table is the whole point
// of this view.
export function SimpleDashboard({ lessons, backlog }: { lessons: CalendarItemOut[]; backlog: BacklogItemOut[] }) {
  ...
  const rows: SimpleRow[] = [...backlog, ...lessons];
