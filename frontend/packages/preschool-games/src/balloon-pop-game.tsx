...
import { getMeQueryKey, useRewardBalloonPop, useRewardBalloonQuiz } from "@school-ahead/api-client/browser/auth/auth";
import { mapApiUserToAuthUser } from "@school-ahead/api-client";
import { prefetchVoice, speak, warmupSpeech, type SpeechLanguage as GameLanguage } from "@school-ahead/api-client";
import {
  EMPTY_MODE_DATA,
  playRecordedSound,
  resolveCardName,
...
import { BalloonQuiz, buildBalloonQuizQuestions, type BalloonQuizQuestion } from "./balloon-quiz";
...
export function BalloonPopGame() {
  ...
  // Every DIAMOND_MILESTONE ruby balloons popped awards 1 Diamond for a
  // signed-in student — an anonymous visitor still sees the ruby count go
  // up, they just don't earn anything (see useDiamondMilestoneReward).
  useDiamondMilestoneReward({
    mode: "count",
    count: score,
    threshold: DIAMOND_MILESTONE,
    rewardMutation: rewardBalloonPop,
  ...
  const handleQuizFinish = (passed: boolean) => {
    ...
    // Anonymous visitors can still play/pass the bonus quiz — they just
    // don't earn a Diamond for it (no student profile to award to).
    if (!user) return;
  ...
  return (
    ...
    >
      ...
      {/* A student in preschool mode has their 💎 balance and the mode switch fixed
          in the page's bottom-right corner (components/preschool/chrome.tsx), so
          this moves out of their way. */}
      {hasCards && (
        <div className="absolute bottom-4 right-4 z-10 flex overflow-hidden rounded-full bg-white p-1 text-sm font-bold shadow-lg ring-2 ring-gray-200 [html[data-headerless]_&]:right-72">
          <button
            type="button"
