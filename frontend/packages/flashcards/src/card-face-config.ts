...
// definition/formula) a flashcard's front and back show — independently
...
// "Тест").
export type CardField = "term" | "translation" | "image" | "definition" | "formula";

export const CARD_FIELDS: CardField[] = ["term", "translation", "image", "definition", "formula"];
...
// Back: the answer (translation + definition + formula) — same "both"
// default the game shipped with before this became configurable.
export const DEFAULT_FRONT_CONFIG: CardFaceConfig = {
  term: true,
  translation: false,
  image: true,
  definition: false,
...
export const DEFAULT_BACK_CONFIG: CardFaceConfig = {
  term: false,
  translation: true,
  image: false,
  definition: true,
...
function isCardFieldAvailable(item: FlashcardItem, hasImage: boolean, field: CardField): boolean {
  ...
  if (field === "formula") return Boolean(item.formula);
  return hasImage;
}
