...
- **Diamond formula.** ~~No exact earning formula is documented for lesson completion, working ahead, or semester closure.~~ — resolved: lesson completion awards +1 diamond on time / +2 if completed ahead of `scheduled_date`, +5 once every Lesson in a Topic is complete, +10 once every Lesson in a SubjectBlock ("semester") is complete (`lessons.services.mark_completed`, see `docs/core/progress.md` section 2). This was implemented directly against `StudentProfile.diamond_balance_cache`, not the `DiamondLedgerEntry` ledger this doc and `02-data-model.md` (decision 3) describe — that app doesn't exist yet, so there's no per-award audit trail or correction support beyond the `TopicCompletionBonus`/`SemesterCompletionBonus` once-per-student-per-group guard rows.
