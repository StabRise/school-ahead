...
- **Diamond formula.** ~~No exact earning formula is documented~~ — resolved and no longer just an architecture assumption: see `docs/core/gamification.md` for the full current earning/spending picture (lesson completion, working-ahead bonus, topic/semester closure, every minigame reward) and `02-data-model.md` decision 3 for why it's a plain counter, not the `DiamondLedgerEntry` ledger this doc originally specced.
...
~~**Admin auto-assignment revocability.**~~ — resolved: every auto-assignment trigger (`tutoring/signals.py`) uses `get_or_create(tutor=..., subject=...)`, which only creates a missing row and never touches `is_active` on an existing one. Setting `TutorSubjectAssignment.is_active = False` on an admin's (or an auto-assigned class teacher's) row sticks and is never silently reactivated. See `02-data-model.md` decision 7.
