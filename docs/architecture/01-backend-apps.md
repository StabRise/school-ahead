...
- **Diamonds/achievements live in `progress`**, decoupled from the lesson state machine — lesson completion triggers a service call into `progress`, never the reverse. Not yet true in practice: the `progress` app doesn't exist, and lesson-completion diamonds are awarded directly inside `lessons.services.mark_completed` against `accounts.StudentProfile.diamond_balance_cache` — see `docs/core/progress.md` section 2 for what's actually built and the gap versus this design.
