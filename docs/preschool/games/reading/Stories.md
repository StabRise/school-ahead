...
навести курсор. Спільний компонент frontend/components/preschool/
story-book.tsx (StoryBook) — той самий і тут, і в грі-нагороді після
уроків (components/preschool/game-choice.tsx), бо обидва місця
...
Гра доступна двома шляхами: /games/stories[/<казка>] — лише для
залогінених учнів-дошкільнят (guard, як і решта ігор), і публічне
дзеркало /reading-game[/<казка>] — без входу в акаунт (посилання
"Вчуся Читати" в шапці сайту для незалогіненого відвідувача, див.
components/header.tsx, middleware.ts's PUBLIC_PATHS). Кожна казка має
власне посилання в обох варіантах (StoriesGamePage приймає basePath —
components/preschool/stories-game.tsx), тож на конкретну казку можна
поділитись напряму.
...
(вони поза зоною прокрутки, components/preschool/stories-game.tsx's
StoryPage).

6. Нагорода

Лише на /games/stories (не на публічному /reading-game — там немає
акаунта, якому нараховувати). Кожне відкрите слово-картка (§5) додає
1 зірочку в лічильник у верхньому правому куті ("⭐ N/5"); кожні 5
зірочок — 💎 діамант, що летить у шапку сайту (той самий
components/flying-diamond.tsx, що й у грі "Потяг"/"Склади"), через
POST /auth/me/stories-game-reward (accounts.services.
award_stories_game_diamond). Лічильник — сесійний (обнуляється, коли
відкриваєш іншу казку чи перезавантажуєш сторінку), без серверної
перевірки — той самий підхід довіри фронтенду, що й в інших
